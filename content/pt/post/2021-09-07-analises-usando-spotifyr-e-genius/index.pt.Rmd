...
Caso alguém realmente queira fazer algo parecido, recomendo que siga os posts referidos ao invés do meu. Não sou grande expert e praticamente copiei um pouco de cada um deles (em alguns casos consegui inclusive copiar errado! tanto que teve coisa que não funcionou).
...
Coloque os dados que você gerou.
```
...
Pelo simples motivo de ser a minha banda favorita do momento. [Ouçam](https://open.spotify.com/embed/artist/3btzEQD6sugImIHPMRgkwV), é muito boa!
...
Dando uma olhada graficamente usando uma das informações dadas pela API. A coluna 'valence', mostra o dado de valência, ou seja, de 'positividade' ou 'regozijo' de uma faixa. Os valores vão de 0 a 1.
...
Nesse caso específico, diferente do pacote usado pro Spotify, não precisamos 
```{r message=FALSE, warning=FALSE}
library(genius)
```
...
```
Dando uma olhada. Fica claro que tem coisa faltando (e muita), e isso é um problema que não consegui resolver.
...
Agora temos todas as letras de todas as músicas dos 3 álbums **SO QUE NÃO!**. Deu pra ver o problema descrito acima.
...
Aqui acontece a mesma coisa. Cada execução, dados diferentes.
```{r}
genius_album(artist = 'Wolf Alice', album = 'Blue Weekend', info = 'all')
```

Já que o pacote genius não funfa (do jeito que eu queria), bora tentar de outra maneira, usando o pacote Geniusr.

### Usando a API do Genius
...
```
Pegue o token lá da página de desenvolvedor do Genius e cole. Se assim não funcionar imediatamente, preste atenção na hora que rodar, o Rstudio pode pedir no console o token.
...
Aqui uma outra forma de fazer o mesmo contornando o problema:
...
albuns_para_remover <- c("Spotify Singles: Holiday", "Demo & Unreleased", "Spotify Singles", "Spotify Sessions",
                         "B-Sides, Demos & Shit", NA, "triple j Like a Version 14", "Ghostbusters (Original Motion Picture Soundtrack)", "")
allLyrics <- allLyrics %>% 
...
tidyLyrics %>%
  ...
  head()
```
*Love* foi a mais frequente (o 'ah' não conta né?)
...
```{r}
# removendo 'ooh's, 'oh's e 'ah's e limitando a palavras que aparecem pelo menos 40 vezes
topFew <- topFew %>% 
  group_by(word) %>%
  mutate(total = sum(n)) %>%
  filter(total >= 40,
...
type = "cairo")
```
Pelo gráfico o 'My Love Is Cool' tem uma maior proporção das palavras mais 
